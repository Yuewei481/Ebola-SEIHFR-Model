...
The hospitalized compartment \(H\) is excluded because hospitalized individuals are assumed to be isolated and therefore do not generate new infections.

The transmission process is represented by the infection subsystem:

\[
\begin{aligned}
E_{t+1} &= E_t + \beta_I S_t I_t + \beta_F S_t F_t - \alpha E_t, \\
I_{t+1} &= I_t + \alpha E_t - (\gamma + \delta) I_t, \\
...
R = \frac{S}{N}
\cdot
\frac{\beta_I \sigma + \beta_F \gamma}
{\sigma(\delta + \gamma)}
\]
